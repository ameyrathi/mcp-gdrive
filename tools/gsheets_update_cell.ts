...
import { GSheetsUpdateCellInput, GSheetsUpdateCellBatchInput, InternalToolResponse } from "./types.js";
...
export async function updateCellsBatch(
  args: GSheetsUpdateCellBatchInput,
): Promise<InternalToolResponse> {
  ...
  const hasFormulas = updates.some((update) => 
    update.value.toString().startsWith("=")
  );
